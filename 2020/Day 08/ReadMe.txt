...
by the value given in the argument. For example, acc +7 would increase the accumulator by 7. 
...
next instruction, jmp +1 would continue to the instruction immediately below it, and 
...
the accumulator from 1 to 2, jmp -4 executes, setting the next instruction to the only 
acc +3. It sets the accumulator to 5, and jmp -3 causes the program to continue back at the 
first acc +1.
